namespace BotMaster.PluginSystem.PluginCreator
{
    ...
    //    private static PluginConnection Create(
    ...
    //                    (s,p)=> PluginConnectionUtils.CreateSendPipe(() => s, p, (s)=>s.IsConnected),
    //                    (s) => PluginConnectionUtils.CreateReceiverPipe(()=>s, (s) => s.IsConnected)
    //                );
    //    }
    ...
    //    internal override PluginConnection Copy() => new IPCPluginInstance(runnersPath, Manifest, createPipe, createSender, createReceiver);
    //}
}